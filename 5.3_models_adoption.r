...
response_var <- "Adoption"

# get the folder with the lasso outputs
counts_folder <- get_lasso_folder("nb", "chatgpt_weekly_usages")
...
features_count_non_imputed <-get_features(paste0(counts_folder, "/selected_features.csv"), base_features, "non_imputed", exclude_features = c(""))
features_count_imputed <- get_features(paste0(counts_folder, "/selected_features.csv"), base_features, "imputed", exclude_features = c(""))
...
save_correlation_matrix(df_non_imputed, features_count_non_imputed, filepath = file.path(output_folder, "corrplot_non_imputed.png"))
save_correlation_matrix(df_imputed, features_count_imputed, filepath = file.path(output_folder, "corrplot_imputed.png"))
...
# da_summ <- data.frame( da_ng$Ranks,  da_mc$Ranks, da_ng$General_Dominance,  da_mc$General_Dominance, da_ng$Standardized, da_mc$Standardized)
