...
# 1. (LASSO) features_zeros_component <- features_binomial
# 2. (LASSO & post-AIC) features_zeros_component <- get_features(paste0(usage_output_folder, "/stepwise_aic_features.csv"), base_features, imputed_option, exclude_features = c(""))
# 3. (RANDOM) features_zeros_component <- c("") # NOT IMPLEMENTED
...
#1. IMPUTED ZERO-INFLATED MODELS###

# fit the zero-inflated models for adoption, sessions and duration
hurdle_nb_adoption_model <- fit_zinf_imputed_model(df, features_zeros_component, features_count_adoption, "Adoption", use_hurdle = TRUE, dist = "negbin")
hurdle_nb_sessions_model <- fit_zinf_imputed_model(df, features_zeros_component, features_count_sessions, "Sessions", use_hurdle = TRUE, dist = "negbin")  
...
save_tab_models(list(binomial_model, nb_no_zeroes_sessions_model, nb_no_zeroes_adoption_model),
    ...
    save_docx = TRUE, show.aic = TRUE, show.aicc = TRUE)
